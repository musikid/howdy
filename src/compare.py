...
def make_snapshot(type):
    ...
    snapshot.generate(snapframes, [
        ...
        "Frames: " + str(frames) + " (" + str(round(frames /
                                                    (time.time() - timings["fr"]), 2)) + "FPS)",
...
capture_failed = config.getboolean(
    "snapshots", "capture_failed", fallback=False)
capture_successful = config.getboolean(
    "snapshots", "capture_successful", fallback=False)
...
while True:
    ...
    if time.time() - timings["fr"] > timeout:
        ...
        if dark_tries == valid_frames:
            ...
            print("Average darkness: " + str(dark_running_total /
                                             max(1, valid_frames)) + ", Threshold: " + str(dark_threshold))
            sys.exit(13)
        else:
            sys.exit(11)
    ...
    if scaling_factor != 1:
        ...
        gsframe = cv2.resize(gsframe, None, fx=scaling_factor,
                             fy=scaling_factor, interpolation=cv2.INTER_AREA)
    ...
    if exposure != -1:
        ...
        # reliable though.
        video_capture.internal.set(
            cv2.CAP_PROP_AUTO_EXPOSURE, 1.0)  # 1 = Manual
